...
class Context(object):
    ...
    def __getitem__(self, key):
        ...
        if j is None and self.r is not None:
            obj = self.outer[key]
    ...
    def get_function(self, pattern):
        for func in self.functions:
            ...
            for i in range(len(pattern)):
                ...
                if (type(item) == ast.Identifier and
                   type(f_item) == ast.Identifier):

                    if item.value != f_item.value:
                        matched = False
                elif not(type(item) == ast.Argument and
                         type(f_item) == ast.Parameter):
                    matched = False
