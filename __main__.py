...
def main():
    parser = argparse.ArgumentParser(
        description="The interpreter for pattern-based-language")
    ...
    parser.add_argument("-p", "--parse", action="store_true",
                        default=False,
                        help="just parse the file - don't execute it")
    ...
    parser.add_argument("-i", "--interactive", action="store_true",
                        default=False,
                        help="enter interactive mode after the file\
has been run")

    parser.add_argument("-v", "--version", action="version",
                        version="pattern-based-language v1.0  --  \
Copyright Zac Garby © 2017")
...
def execute(text, print_result, ctx):
    ...
    if len(parser.errors) > 0:
        ...
        should_print = print_result and type(result) != o.Null
        is_err = type(result) == o.Error

        if should_print or is_err:
            print(result)
