...
def evaluate(node, ctx):
    ...
    # Constructs
    if t == ast.Program:
        return eval_program(node, ctx)
    if t == ast.BlockStatement:
        return eval_block_stmt(node, ctx)
    if t == ast.ExpressionStatement:
        return evaluate(node.expr, ctx)
    if t == ast.IfExpression:
        return eval_if(node, ctx)
    if t == ast.WhileLoop:
        return eval_while_loop(node, ctx)
    if t == ast.ForLoop:
        return eval_for_loop(node, ctx)
    ...
    if t == ast.Identifier:
        return eval_id(node, ctx)
    if t == ast.BlockLiteral:
        return eval_block(node, ctx)

    if t == ast.NextStatement:
        return NEXT
    if t == ast.BreakStatement:
        return BREAK

    # Functions
    if t == ast.FunctionDefinition:
        return eval_function_def(node, ctx)
    if t == ast.FunctionCall:
        return eval_function_call(node, ctx)
...
def is_err(o):
    return False if o is None else type(o) == obj.Error
...
def eval_block_stmt(block, ctx):
    ...
    for stmt in block.statements:
        ...
        if t is not None and result.type in [
                obj.RETURN_VALUE,
                obj.ERROR,
                obj.NEXT,
                obj.BREAK]:
...
def eval_prefix(op, right):
    if op == "-":
        return eval_minus_prefix(right)
    if op == "+":
        return right
...
def eval_infix(op, left, right, ctx):
    ...
    # Boolean operators
    if op == "&&":
        return bool_obj(is_truthy(left) and is_truthy(right))
    if op == "||":
        return bool_obj(is_truthy(left) or is_truthy(right))
    if op == "==":
        return bool_obj(left == right)
    if op == "!=":
        return bool_obj(left != right)
...
def eval_collection_infix(op, left, right, ctx):
    ...
    if op == "+":
        return type(left)(l + r)
    if op == "-":
        return type(left)([e for e in l if e not in r])
    if op in "&&":
        return type(left)([e for e in l if e in r])
...
def eval_function_call(node, ctx):
    ...
    p_string = "".join((e.value if type(
        e) == ast.Identifier else "$") + " " for e in node.pattern)[:-1]
...
def eval_for_loop(node, ctx):
    ...
    if s is None:
        return err("cannot use a for loop over a collection of type %s" %
                   collection.type)
