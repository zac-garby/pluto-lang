...
class Parser(object):
    ...
    def next(self):
        ...
        if self.peek_tok.type == token.ILLEGAL:
            self.err("illegal token found: `%s`" % self.peek_tok.literal,
                     self.peek_tok.start, self.peek_tok.end)
    ...
    def parse_expr(self, precedence):
        ...
        while (not self.peek_is(token.SEMI) and
    ...
    def parse_grouped_expr(self):
        ...
        if self.peek_is(token.COMMA):
            ...
            expr = ast.Tuple(expr.token, [expr] +
    ...
    def parse_function_call(self, first=None):
        ...
        while (self.peek_in(arg_tokens) or
               self.peek_in(token.keywords.values())):
            self.next()
            ...
            def arg(val): return ast.Argument(self.cur_tok, val)
    ...
    def parse_infix(self, left):
        expr = ast.InfixExpression(
            self.cur_tok, self.cur_tok.literal, left, None)
